...
bool IsDmlFlaggedMainTank(Player* player)
{
    if (!player)
        return false;
    ...
    for (Group::member_citerator itr = slots.begin(); itr != slots.end(); ++itr)
    {
        if (itr->flags & MEMBER_FLAG_MAINTANK)
            return itr->guid == player->GetGUID();
    }

    return false;
}

Player* GetDmlFlaggedMainTank(Group* group)
{
    if (!group)
        return nullptr;
    ...
    for (GroupReference* ref = group->GetFirstMember(); ref; ref = ref->next())
    {
        ...
        if (member && member->GetGUID() == mainTankGuid)
            return member;
    }
...
bool ShouldDmlBlockPlayerbotPetTaunt(Creature* pet, Unit* target, SpellInfo const* spellInfo)
{
    if (!pet || !target || !IsDmlTauntSpell(spellInfo))
        return false;

    Unit* ownerUnit = pet->GetCharmerOrOwner();
    Player* owner = ownerUnit ? ownerUnit->ToPlayer() : nullptr;
    if (!owner || !owner->GetSession() || !owner->GetSession()->IsBot())
        return false;

    // Single-target taunt: never pull a hostile unit off the designated main tank.
    if (Unit* victim = target->GetVictim())
    {
        if (Player* victimPlayer = victim->ToPlayer())
            if (IsDmlFlaggedMainTank(victimPlayer))
                return true;
    }

    // Area pet taunts (for example Suffering-style effects) can catch a main
    // tank target even when the pet's primary victim is different. Protect any
    // main-tank attacker that is actually inside the taunt effect radius.
    float tauntRadius = 0.0f;
    for (SpellEffectInfo const& effect : spellInfo->Effects)
    {
        if (effect.IsAura(SPELL_AURA_MOD_TAUNT) || effect.IsEffect(SPELL_EFFECT_ATTACK_ME))
        {
            float radius = effect.CalcRadius(pet);
            if (radius > tauntRadius)
                tauntRadius = radius;
        }
    }
    ...
    Player* mainTank = GetDmlFlaggedMainTank(owner->GetGroup());
    if (!mainTank)
        return false;

    for (Unit* attacker : mainTank->getAttackers())
    {
        if (attacker && attacker->IsAlive() && pet->GetDistance(attacker) <= tauntRadius + 1.0f)
            return true;
    }
...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING))
    {
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            // DML: playerbot pets may DPS normally, but their autonomous taunts
            // must never steal an enemy from the raid-designated main tank.
            // Explicit takeaggro uses the forced-spell path above, so it is the
            // intentional one-shot exception to this autocast protection.
            if (ShouldDmlBlockPlayerbotPetTaunt(me, me->GetVictim(), spellInfo))
                continue;
